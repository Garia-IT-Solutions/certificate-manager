      ...
      <div className="relative z-10">
        ...
        <div className="mt-6 flex items-center gap-6">
          <div className="flex flex-col">
            ...
            <span className="text-xs font-bold text-zinc-800 dark:text-zinc-200">{lastVessel}</span>
          </div>
          <div className="w-px h-6 bg-zinc-200 dark:bg-zinc-800" />
          <div className="flex flex-col">
            <span className="text-[9px] font-bold uppercase text-zinc-400 tracking-wider">Rank</span>
            <span className="text-xs font-bold text-zinc-800 dark:text-zinc-200">{lastRank}</span>
          </div>
        </div>
      </div>
    </div>
          ...
          {recentVoyages.length > 0 ? recentVoyages.map((voyage, i) => (
            <div key={i} className="flex items-center justify-between p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors group cursor-pointer">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-bold text-zinc-900 dark:text-white">{voyage.vesselName}</p>
                  <p className="text-[10px] text-zinc-500 font-medium mt-0.5">{voyage.type} • {voyage.dwt?.toLocaleString()} DWT</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-sm font-mono font-bold text-zinc-900 dark:text-white">{voyage.days} Days</p>
                <p className="text-[10px] text-zinc-400 mt-0.5">as {voyage.rank}</p>
              </div>
            </div>
          )) : (
  ...
  return (
    <div className="flex flex-col justify-end h-full">
      ...
      <div className="flex items-center gap-2">
        ...
        <p className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide">{status}</p>
      </div>
    ...
    <div className="space-y-6">
      <div className="p-5 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-bold text-zinc-900 dark:text-white">{validCount} / {totalCount} Valid</p>
            <p className="text-xs text-zinc-500 mt-0.5">{totalCount - validCount} Document{totalCount - validCount !== 1 && 's'} require{totalCount - validCount === 1 && 's'} attention</p>
          </div>
        </div>
      </div>
            ...
            <div key={i} className="flex justify-between items-center p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-500">{doc.expiryDate || 'No Expiry'}</p>
                </div>
              </div>
              <span className={cn("text-[10px] font-bold uppercase px-2.5 py-1 rounded-md bg-zinc-100 dark:bg-zinc-800", doc.status === 'VALID' ? 'text-emerald-500' : 'text-orange-500')}>{doc.status}</span>
            </div>
          )) : (
            <div className="text-center py-4 text-zinc-400 text-xs">No documents found</div>
          )}
        </div>
      </div>
...
function CertificatesContent({ data }: { data?: DashboardData['certificates'] }) {
  ...
  return (
    <div className="space-y-6">
      <div className={cn(
        "p-5 rounded-2xl border flex gap-5 items-start",
        hasIssues
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
            ...
            <p className="text-[10px] font-bold text-zinc-400 uppercase mt-1">Expired</p>
          </div>
        </div>
      </div>
...
function NRIStatusSummary({ data }: { data?: DashboardData['nriStatus'] }) {
  ...
  // const period = data ? `${data.startDate.split(' ')[2]} - ${data.endDate.split(' ')[2]}` : 'FY';

  return (
    <div className="flex flex-col h-full w-full px-6 py-5 relative overflow-hidden">
      <div className="absolute -right-5 -top-5 h-24 w-24 bg-white/10 blur-2xl rounded-full pointer-events-none" />
      ...
      <div className="flex items-start justify-between relative z-10">
        <div className="flex items-center gap-2">
          ...
          <span className={cn(
            "px-2 py-1 rounded-md text-[9px] font-extrabold uppercase tracking-widest border border-white/10",
      ...
      <div className="flex-1 flex flex-col justify-center text-center">
        ...
        <p className="text-[10px] text-white/60 font-medium mt-1 uppercase tracking-wider">
          Days Completed
        </p>
      </div>
...
function NRIStatusDetail({ data }: { data?: DashboardData['nriStatus'] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="p-6 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 text-center relative overflow-hidden">
        <div className="relative z-10">
          <h3 className="text-xs font-bold uppercase text-zinc-400 tracking-widest mb-1">NRI Status Validity</h3>
          <p className={cn("text-3xl font-light tracking-tight", isRetained ? "text-emerald-600 dark:text-emerald-400" : "text-orange-600 dark:text-orange-400")}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex flex-col items-center justify-center bg-white dark:bg-zinc-950">
      ...
      <div className="px-4 py-3 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30 flex items-start gap-3">
        <Info size={16} className="text-blue-500 mt-0.5 shrink-0" />
        <div>
          <p className="text-xs font-bold text-blue-700 dark:text-blue-400 mb-0.5">Calculation Period</p>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Derive notifications from alerts
    ...
    <div className="min-h-screen w-full bg-transparent selection:bg-orange-500 selection:text-white pb-32">
      <main className="relative mx-auto max-w-[1400px] p-6 md:p-8">
          ...
          <div className="flex items-center gap-4 bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-md p-2 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm z-20">
            <div className="flex items-center gap-3 px-3">
              ...
              <div>
                ...
                <p className="text-[9px] text-zinc-400 font-mono"><LiveDate /></p>
              </div>
            </div>
            <ThemeToggle />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-4">
          ...
          {/* Row 3 Right: NRI Status (Replaces Alert) */}
          <div className="md:col-span-2 h-[160px]">
            <BentoCard
              id="nri-status"
              title=""
